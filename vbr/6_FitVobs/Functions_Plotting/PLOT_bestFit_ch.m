...
%set(gcf, 'PaperSize', [4 4]); % BH added this, but it did not help
...
%    plot(V_VBR-2*Std_V,Z_km,'color',[0.2 0.2 0.2])
   ...
   xlim([3.5 5.5]);
   ylim(ylimits)
   ylabel('Z (km)','fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex')
   xlabel('Vs (km/s)','fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex');
   ...
   title(title_name,...
       'fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex')
...
axes('position', plot_B,'box','on'); hold on;
    ...
    ylim(ylimits)
    set(gca, 'YTick', []);
    title('% Difference','fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex');
...
if VarInfo.Var2_n > 1
    ...
    set(h,'position',pos)
    colormap(gray);
    title('$\log_{10}$ misfit','fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex');
...
%%%  Small line plot of min misfit at each Tpot, above 2D plot
    ...
    E_axis=axes('position', plot_E);
      ...
      ylabel(['min misfit'], 'Interpreter', 'latex')
      xlim(xlimits)
...
elseif VarInfo.Var2_n==1
    ...
    xlabel([VarInfo.Var1_name ' (' VarInfo.Var1_units ')'],...
        'fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex');
    ylabel('Misfit','fontname','Times New Roman','fontsize',LBLFNT, 'Interpreter', 'latex');
