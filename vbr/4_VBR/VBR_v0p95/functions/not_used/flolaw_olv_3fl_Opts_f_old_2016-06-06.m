...
% get the flowlaw parameter structure, FP  
  FP = initialize_flow_laws(T,fH2O,comp,FlowLawOpts); 
...
function FP = initialize_flow_laws(T,fH2O,comp,Params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   Rhe = flow_law_consts; 
  Rhe = flow_law_constants;

% effects of melt [1,2,3] for [diffusion, dislocation and gbs] creep
  speedfac_vec = [speedfacdiff; 1.0; speedfacdiff/2];
  phi_cs = [phi_c; phi_c; phi_c]; % look!! a row of phi_cs !  
    
% initialize constants 
  nmechs = Params.n_mech; % number of mechanisms
  FP = struct();
  for i_mech=1:nmechs
   FP(i_mech).A = zeros(size(T)); % pre-exponential
   FP(i_mech).n = FP.A ; % stress exponent
   FP(i_mech).p = FP.A ; % grain size exponent
   FP(i_mech).r = FP.A; % water fugacity exponent
   FP(i_mech).Q = FP.A; % activation energy
   FP(i_mech).V = FP.A; % activation volume
   FP(i_mech).alf = FP.A; % standard melt fraction exponential factor   
  end
  fields = fieldnames(FP(1)); % the above field names
...
%          no sensitivity of disl. to phic.
  ...
  for i_mech = 1:nmechs
      FP(i_mech).speedfac = speedfac_vec(i_mech);
      FP(i_mech).phi_c = phi_cs(i_mech);
  end
