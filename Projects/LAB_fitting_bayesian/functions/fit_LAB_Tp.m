function zPlate_fit = fit_LAB_Tp(VbrBoxFile, seismic_obs, set_Tp, q_method)
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %
...
% seismic LAB depth than on temperature estimates.
  ...
  %
  % cleanup
  zPlate_fit.resids=resids;
  zPlate_fit.bestJoint=best;
  zPlate_fit.predictions=preds;
end
% calculate joint residual
...
% fits.Res_Vs_N = fits.Res_Vs_adavg_mat./max(fits.Res_Vs_adavg_mat(:));
...
% f_band = VbrBox(1,1).Frames(end).VBR.in.SV.f;
%
% clc;
% fprintf(['\n\nTpot: %.0f - %.0f C\nzPlate: %.0f - %.0f km\n' ...
...
%         Qz_interp = interp1(Z_km,Qs_mnstd(:,1),Z_km_interp); % Q from VBR
%
