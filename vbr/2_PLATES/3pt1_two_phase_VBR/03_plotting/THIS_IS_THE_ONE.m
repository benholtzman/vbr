...
% choose a thing
  ...
  loaddir0=[hmdir '/Dropbox/Research/0_Boxes/old_boxes']; % the closet to store the box in 
  loadbase = '2016-04-13-Tpot_vs_Vbg_var'; % boxes end up named ['Box_' savebase]
...
% (plot 2) time evolution of single run (with heat flow)
   Targetv1 = 1500; % variable 1 value
   Targetv2 = 5; % variable 2 value (will find the closest box index)
   t_skip = 4; % time step skip for time evolution plot
   ylims=[0 120];
...
%% ------------------------------------------------------------------------ 
   if strcmp(recalc,'yes')
       for iBox = 1:numel(Box)
           Box(iBox).run_info=Box(iBox).Movie.info; 
           Box(iBox).Frames=Box(iBox).Movie.Frames; 
       end
