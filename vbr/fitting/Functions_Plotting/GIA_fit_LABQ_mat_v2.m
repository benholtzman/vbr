function GIA_fit_LABQ_mat_v2(varargin)
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %
%  Fit the measured LAB and Vs to find reasonable models       %
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %


% Set built in values for potential inputs
if isempty(varargin); inpt.set = 'auto'; else; inpt = varargin{1}; end
inpt_names = {'boxname','basedir','q_method','hot','cold',...
    'Q_LAB','eta_LAB','per_bw_max','per_bw_min','temps'};
for iin = 1:length(inpt_names)
    if isfield(inpt,inpt_names{iin}); continue; end
    switch inpt_names{iin}
        case 'boxname'
            inpt.boxname = '2018-07-02-CHbox'; % '2018-07-02-BigBox';
        case 'basedir'
            %inpt.basedir = '/Users/ben/0_vbr_projects/VBRcloset/';
            inpt.basedir = 'C:/Users/Emily/Documents/VBR/Boxes/';
        case 'q_method'
            inpt.q_method = 'AndradePsP';
        case 'hot'
            inpt.hot.zLAB = 80;
            inpt.hot.Vs_adavg = 4.6;
        case 'cold'
            inpt.cold.zLAB = 180;
            inpt.cold.Vs_adavg = 4.7;
        case 'temps'
            inpt.temps = {'hot','cold'};
        case 'Q_LAB'
            inpt.Q_LAB = 800;
        case 'eta_LAB'
            inpt.eta_LAB = 1e24;  
        case 'per_bw_max' % max period to use for fitting (s)
            inpt.per_bw_max = 30;
        case 'per_bw_min' % min period to use for fitting (s)
            inpt.per_bw_min = 10;
    end
end
inpt.boxobj = ['Box_' inpt.boxname '_VBR_py.mat'];
inpt.path = [inpt.basedir inpt.boxname '/'];
...
% Q(Z) plot
plot_Q_profiles(Layout.Qz, Box, fits, inpt, i_fmin, i_fmax, inpt.q_method);
% Vs(Z) plot
plot_Vs_profiles(Layout.Vsz, Box, fits, inpt, i_fmin, i_fmax, inpt.q_method);
% Residuals (top row - hot)
residPlots(Layout.rh, Tpot_vec, zPlate_vec, fits, 'hot', [1 0 0]);
% Residuals (bottom row - cold)
residPlots(Layout.rb, Tpot_vec, zPlate_vec, fits, 'cold', [0 0 1]);

ij_best=fits.hot.ij_best;
fprintf('\n\nHOT   Tpot: %.0f; zPlate: %.0f',...
    Tpot_vec(ij_best(1)),zPlate_vec(ij_best(2)));
ij_best=fits.cold.ij_best;
fprintf('\nCOLD   Tpot: %.0f; zPlate: %.0f\n\n',...
    Tpot_vec(ij_best(1)),zPlate_vec(ij_best(2)));
...
function plot_Q_profiles(layout, Box, fits, obs, ...
    i_fmin, i_fmax, q_method)
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %
...
for temp = {'cold','hot'}
   ...
   p3 = plot(axlims(1,:), obs.(temp{:}).zLAB*[1 1],...
...
function plot_Vs_profiles(layout, Box, fits, inpt, ...
    i_fmin, i_fmax, q_method)
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %
...
function residPlots(layouts,Tpot_vec,zPlate_vec,fits,...
    temp,clr)
%  *   *   *   *   *   *   *   *   *   *   *   *   *   *   *   %
